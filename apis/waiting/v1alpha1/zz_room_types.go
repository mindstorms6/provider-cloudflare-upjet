...
type RoomInitParameters struct {
	...
	// (Attributes) Configures cookie attributes for the waiting room cookie. This encrypted cookie stores a user's status in the waiting room, such as queue position. (see below for nested schema)
...
type Room struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.additionalRoutes) || (has(self.initProvider) && has(self.initProvider.additionalRoutes))",message="spec.forProvider.additionalRoutes is a required parameter"
